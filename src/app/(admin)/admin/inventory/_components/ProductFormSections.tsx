...
export function CostingSection({
  ...
  const costingHref = productId
    ? `/admin/finance/costing?productId=${productId}`
    : "/admin/finance/costing";
    ...
    async function load() {
      ...
      try {
        const [balanceResponse, movementResponse] = await Promise.all([
          fetch(`/api/admin/finance/inventory/balances?itemType=product&itemId=${encodeURIComponent(productId)}`, { cache: "no-store" }),
          fetch(`/api/admin/finance/inventory/movements?itemType=product&itemId=${encodeURIComponent(productId)}`, { cache: "no-store" }),
        ]);
        if (cancelled) return;
      ...
      {recipe ? (
        <div className="space-y-3">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <Link href={`/admin/finance/costing?productId=${productId}`} className="rounded-lg bg-amber-600 px-3 py-2 text-xs font-bold text-white transition hover:bg-amber-700">Chỉnh BOM</Link>
          </div>
        ...
        <div className="rounded-xl border border-dashed border-amber-200 bg-amber-50/40 p-4"><p className="font-bold text-neutral-800">Chưa có BOM hoạt động</p><p className="mt-1 text-sm text-neutral-500">Thiết lập định mức để giá vốn tự cập nhật theo giá nguyên liệu.</p><Link href={`/admin/finance/costing?productId=${productId}`} className="mt-3 inline-flex rounded-lg border border-amber-200 bg-white px-3 py-2 text-xs font-bold text-amber-800 hover:bg-amber-50">Lập BOM</Link></div>
      )}
    </FormSection>
  );
}
...
export function ProductionBomEditor({ productId }: { productId: string }) {
  ...
  const saveDraft = async () => {
    ...
    if (!validLines.length || draft.yieldQuantity < 1) {
      setMessage("Cần có ít nhất một nguyên liệu và sản lượng mẻ hợp lệ.");
      return;
    }
    setIsSaving(true);
    setMessage(null);
    try {
      ...
      setMessage(`Đã lưu BOM v${created.version} ở trạng thái nháp. Kích hoạt khi sẵn sàng áp dụng.`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Không thể lưu BOM nháp.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const activateDraft = async () => {
    ...
    try {
      ...
      setMessage("BOM đã được kích hoạt; phiên bản cũ vẫn được lưu trong lịch sử.");
      ...
      setMessage(error instanceof Error ? error.message : "Không thể kích hoạt BOM.");
    } finally {
      setIsSaving(false);
    }
  };

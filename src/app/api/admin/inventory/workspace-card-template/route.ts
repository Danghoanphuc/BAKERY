...
import { listAdminRecords, updateAdminRecord } from "@/lib/admin-record-store";
...
export async function POST(request: Request) {
  ...
  const products = await listAdminRecords("products") as unknown as Product[];
  ...
  await Promise.all(products.map(async (product) => {
    ...
    if (!changed) return;
    await updateAdminRecord("products", product.id, { workspaceCards });
    updated += 1;
  }));

...
import { deleteWholesaleRecord, getWholesaleRecord, updateWholesaleRecord } from "@/lib/wholesale-admin-store";
import {
...
export async function PUT(
  ...
  try {
    ...
    await updateWholesaleRecord("customers", id, isAdmin ? data : customerUpdatePayload(data));
...
export async function GET(
  ...
  try {
    ...
    const { id } = await context.params;
    const customer = await getWholesaleRecord("customers", id);
...
export async function DELETE(
  ...
  try {
    ...
    await deleteWholesaleRecord("customers", id);
    return NextResponse.json({ ok: true });
  } catch (error) {

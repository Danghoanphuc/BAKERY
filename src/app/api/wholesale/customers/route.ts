...
import { createWholesaleCustomer, getWholesaleRecord, listWholesaleRecords } from "@/lib/wholesale-admin-store";
...
export async function GET(request: Request) {
  ...
  try {
    const customers = await listWholesaleRecords("customers");
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingCustomer = await getWholesaleRecord("customers", phone);
    ...
    let customer;
    try {
      customer = await createWholesaleCustomer({
        name,
        phone,

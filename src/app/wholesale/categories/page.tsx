...
export default function CategoriesPage() {
  ...
  async function loadData() {
    try {
      setIsLoading(true);
      const res = await fetch("/api/wholesale/categories");
      if (!res.ok) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(
        editingCategory
          ? `/api/wholesale/categories/${editingCategory.id}`
  ...
  async function toggleVisibility(category: Category) {
    ...
    try {
      const response = await fetch(`/api/wholesale/categories/${category.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          isVisible: !(category.isVisible ?? true),
  ...
  async function saveOrder(
    ...
    try {
      const response = await fetch("/api/wholesale/categories/reorder", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function moveProductsAndDelete() {
    ...
    try {
      const moveResponse = await fetch(
        `/api/wholesale/categories/${deleteCandidate.id}/move-products`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const deleteResponse = await fetch(`/api/wholesale/categories/${deleteCandidate.id}`, {
        method: "DELETE",
      });

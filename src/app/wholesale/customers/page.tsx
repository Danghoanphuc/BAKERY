...
} from "@/features/wholesale-admin/customers";
  ...
  async function loadData() {
    try {
      ...
      const [customersResponse, ordersResponse] = await Promise.all([
        fetch("/api/wholesale/customers", { cache: "no-store" }),
      ...
      if (!ordersResponse.ok) throw new Error("KhĂ´ng thá»ƒ táº£i Ä‘Æ¡n hĂ ng");

      setCustomers((await customersResponse.json()) as Customer[]);
      const orderPage = await ordersResponse.json() as { orders: Order[] };
  ...
  return (
    <div className="space-y-6">
      ...
      <CustomerTable
        ...
        onOpenCustomer={(id) => router.push(`/wholesale/customers/${id}`)}
      />

...
export default function ManagementPage() {
  ...
  const load = useCallback(async () => {
    const [centerRes, policyRes, budgetRes, summaryRes] = await Promise.all([
      fetch("/api/wholesale/finance/cost-centers", { cache: "no-store" }),
      fetch("/api/wholesale/finance/allocation-policies", { cache: "no-store" }),
      fetch(`/api/wholesale/finance/budgets?period=${period}`, { cache: "no-store" }),
      fetch(`/api/wholesale/finance/management-summary?period=${period}`, { cache: "no-store" }),
    ]);
  ...
  async function createCenter(event: FormEvent) {
    ...
    const response = await post("/api/wholesale/finance/cost-centers", {
  ...
  async function createBudget(event: FormEvent) {
    event.preventDefault(); setSaving(true);
    const response = await post("/api/wholesale/finance/budgets", {
  ...
  async function approveBudget(id: string) {
    setSaving(true); const response = await fetch(`/api/wholesale/finance/budgets/${id}/approve`, { method: "POST" });
  ...
  async function createPolicy(event: FormEvent) {
    ...
    const response = await post("/api/wholesale/finance/allocation-policies", {
  ...
  async function activatePolicy(id: string) {
    setSaving(true); const response = await fetch(`/api/wholesale/finance/allocation-policies/${id}/activate`, { method: "POST" });
  ...
  async function runSimulation(event: FormEvent) {
    ...
    if (!summary) return;
    setSaving(true);
    const response = await post("/api/wholesale/finance/simulate", {

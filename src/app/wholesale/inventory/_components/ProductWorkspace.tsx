...
function useProductLedgerStock(productId: string, enabled: boolean): LedgerStockState {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await fetch(`/api/wholesale/finance/inventory/balances?itemType=product&itemId=${encodeURIComponent(productId)}`, { cache: "no-store" });
...
function useProductAnalytics(productId: string, days: number): ProductAnalyticsState {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const response = await fetch(`/api/wholesale/products/${encodeURIComponent(productId)}/analytics?days=${days}`, { cache: "no-store" });

...
function NumberInput({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label className="block text-xs font-black text-neutral-600">{label}<FormattedNumberInput min={0} value={value} onValueChange={(nextValue) => onChange(nextValue ?? 0)} className="mt-1 h-10 w-full rounded-xl border border-neutral-200 px-3 text-sm font-semibold outline-none focus:border-[#b84a39]" /></label>; }

...
export default function MarketingPage() {
  ...
  async function loadMarketing() {
    try {
      setIsLoading(true);
      const response = await fetch("/api/wholesale/marketing");
  ...
  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/wholesale/marketing/settings", {
        method: "PUT",
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            href="/wholesale/marketing/vouchers/new"
  ...
  if (!campaigns.length) {
    return (
      <div className="rounded-lg border border-neutral-200 bg-white p-10 text-center">
        ...
        <Link href="/wholesale/marketing/vouchers/new" className="mt-5 inline-flex h-10 items-center rounded-lg bg-brand-500 px-4 text-sm font-semibold text-white">
          Tạo chương trình
        </Link>
      </div>
    );
              ...
              return (
                <tr key={campaign.id} className="hover:bg-neutral-50">
                  <td className="px-4 py-4">
                    <Link href={`/wholesale/marketing/vouchers/${campaign.id}`} className="font-bold text-neutral-950 hover:text-brand-600">
                      {campaign.name}
                    </Link>

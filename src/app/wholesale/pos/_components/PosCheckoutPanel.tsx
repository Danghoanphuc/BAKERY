...
export function PosCheckoutPanel({
  ...
  return (
    <section
      className={clsx(
        "flex flex-col gap-3 p-3 lg:p-4",
        stackedOnTablet ? "min-h-0 xl:min-h-full" : "min-h-full",
      )}
    >
      {children}
        ...
        <div className="rounded-2xl border border-[#eadbcc] bg-white p-3">
          ...
          <div className="mt-2 grid grid-cols-[minmax(0,1fr)_auto] gap-2">
            ...
            <button
              ...
              className="flex h-11 items-center justify-center rounded-xl border border-emerald-200 bg-emerald-50 px-3 text-xs font-black text-emerald-700 transition hover:bg-emerald-100"
            >
      ...
      {isNoteOpen ? (
        <div className="rounded-2xl border border-[#eadbcc] bg-white p-3">
          <div className="flex items-center justify-between gap-2">
            <span className="flex items-center gap-2 text-xs font-black text-[#65483a]">
            ...
            <button
              ...
              className="flex h-9 items-center gap-1 rounded-lg px-2 text-xs font-black text-[#7b6254] hover:bg-[#fffaf6]"
            >
        ...
        <button
          ...
          className="flex min-h-11 w-full items-center justify-between gap-3 rounded-xl border border-[#eadbcc] bg-white px-3 text-left text-xs font-black text-[#65483a] transition hover:border-[#b84a39]/40"
        >
          <span className="flex min-w-0 items-center gap-2">
            <MessageSquare className="h-3.5 w-3.5 shrink-0 text-[#9b8171]" />
            <span className="truncate">{orderNote || "Thêm ghi chú đơn"}</span>
          </span>
          <ChevronDown className="h-3.5 w-3.5 shrink-0" />
        </button>
      )}

      <div className="sticky bottom-0 z-10 mt-auto space-y-2 bg-white pt-2">
        <div className="space-y-2 rounded-2xl bg-[#fffaf6] p-3 ring-1 ring-[#f0e1d2]">
          {selectedVoucher && voucherPricing.isEligible && (
            <>

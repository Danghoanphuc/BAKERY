...
export default function VoucherCampaignDetailPage({
    ...
    async function loadCampaign() {
      try {
        ...
        const redemptionsResponse = await fetch(
          `/api/wholesale/vouchers/${id}/redemptions`,
        );
        const redemptionsPayload = redemptionsResponse.ok
          ? await redemptionsResponse.json()
          : { redemptions: [] };
        const lifecycleResponse = await fetch(`/api/wholesale/vouchers/${id}/lifecycle`, { cache: "no-store" });
  ...
  async function changeStatus(status: MarketingCampaign["status"], action: string) {
    ...
    try {
      const response = await fetch(`/api/wholesale/marketing/${campaign.id}`, {
      ...
      const lifecycle = await fetch(`/api/wholesale/vouchers/${campaign.id}/lifecycle`, { cache: "no-store" }).then((item) => item.json());
  ...
  return (
    <div className="space-y-5">
      <div>
        <Link
          href="/wholesale/marketing/vouchers"

...
export function GrowthStudio() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        const response = await fetch("/api/admin/growth-studio", { cache: "no-store" });
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          const response = await fetch(`/api/admin/growth-studio/${workspaceToSave.id}`, {
            method: "PUT",
  ...
  async function createCustomerProfile(templateId: GrowthWorkspaceTemplateId = "product_social_launch_v1") {
    ...
    setIsCreating(true);
    try {
      const response = await fetch("/api/admin/growth-studio", {
  ...
  async function runNode(nodeId: string) {
    ...
    try {
      const response = await fetch("/api/admin/growth-studio/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function performWorkspaceAction(body: Record<string, unknown>, workspaceId = activeWorkspace?.id) {
    ...
    const response = await fetch(`/api/admin/growth-studio/${workspaceId}/actions`, {

...
} from "@/app/(admin)/admin/vouchers/_lib/voucher-admin";
...
export function GuidedVoucherCreator() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/vouchers/ai-workspace?campaignId=${encodeURIComponent(id)}`, { cache: "no-store" })
      .then(async (response) => {
  ...
  async function askAi(content: string, nextDraft: VoucherDraft = draft) {
    ...
    try {
      const response = await fetch("/api/admin/vouchers/ai-guide", {
  ...
  async function save(status: "draft" | "active") {
    ...
    try {
      const response = await fetch(campaignId ? `/api/marketing/${campaignId}` : "/api/marketing", {
      ...
      window.location.href = "/admin/marketing/vouchers";
...
function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label className="block text-xs font-bold text-neutral-600">{label}<FormattedNumberInput min={0} value={value} onValueChange={(nextValue) => onChange(nextValue ?? 0)} className="mt-1 h-10 w-full rounded-lg border border-neutral-200 px-3 text-sm outline-none focus:border-brand-500" /></label>; }

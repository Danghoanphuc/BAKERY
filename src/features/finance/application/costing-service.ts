...
export async function addIngredient(
  input: Omit<FinanceIngredient, "id" | "code" | "createdAt" | "updatedAt"> & {
    groupCode?: string;
  ...
  if (typeof input.name !== "string" ||
      !input.name.trim() || !baseUnits.has(input.baseUnit) ||
...
export async function editIngredient(
  ingredientId: string,
  patch: { name?: unknown; isActive?: unknown; code?: unknown; baseUnit?: unknown },
  actor: string,
) {
